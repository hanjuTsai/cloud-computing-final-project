...
def lambda_handler(event, context):
    ...
    query = get_query_json(
      ...
      price_levels=["$" * i for i in range(1, len(price_level)+1)], # need pre-processing
      cuisine_type= cuisine_type
    )
    
    response = requests.get(elastic_url, data = json.dumps(query), auth=('master','Master-6998'), verify=False, headers = headers)
